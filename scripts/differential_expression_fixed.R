...
# This script performs differential expression analysis using limma
# It can handle the fixed design matrix with control/treatment classification
...
find_design_matrix <- function(base_dir, gse_id) {
  ...
  possible_locations <- c(
    ...
    # Check for fixed design matrix
    file.path(base_dir, "sample_design", "sample_design", "design_matrices", paste0(gse_id, "_design_matrix_fixed.txt")),
    file.path(path.expand("~/scratch/B16F10"), "sample_design", "sample_design", "design_matrices", paste0(gse_id, "_design_matrix_fixed.txt"))
  )
...
perform_de_analysis <- function(gse_id) {
  ...
  results_file <- file.path(output_dir, paste0(gse_id, "_de_results_fixed.rds"))
  ...
  # Check if we have a Treatment column in the design matrix
  if ("Treatment" %in% colnames(design_matrix)) {
    cat("Using Treatment column from design matrix...\n")
    
    # Create factor from treatment information
    treatment_factor <- factor(design_matrix$Treatment)
    
    # Print treatment factor levels
    cat("Treatment factor levels:\n")
    print(levels(treatment_factor))
    
    # Check if we have at least two levels
    if (length(levels(treatment_factor)) < 2) {
      cat("Treatment factor has fewer than 2 levels. Cannot proceed.\n")
      return(FALSE)
    }
    
    # Create design matrix
    design <- model.matrix(~0 + treatment_factor)
    colnames(design) <- levels(treatment_factor)
  } else {
    ...
    if (length(levels(group_factor)) < 2) {
      cat("Group factor has fewer than 2 levels. Cannot proceed.\n")
      return(FALSE)
    }
  ...
  for (contrast_name in names(contrasts)) {
    ...
    # Save results
    res_file <- file.path(output_dir, paste0(gse_id, "_", contrast_name, "_de_results_fixed.txt"))
    write.table(res, res_file, sep = "\t", quote = FALSE)
    cat(sprintf("Saved results to: %s\n", res_file))
    
    # Create MA plot
    cat(sprintf("Creating MA plot for %s...\n", contrast_name))
    pdf(file.path(output_dir, paste0(gse_id, "_", contrast_name, "_ma_plot_fixed.pdf")))
    plotMA(fit2, coef = 1)
    dev.off()
    
    # Create volcano plot
    cat(sprintf("Creating volcano plot for %s...\n", contrast_name))
    pdf(file.path(output_dir, paste0(gse_id, "_", contrast_name, "_volcano_plot_fixed.pdf")))
  ...
  if (length(results) > 0) {
    ...
    if (length(all_de_genes) > 0) {
      ...
      # Create heatmap
      pdf(file.path(output_dir, paste0(gse_id, "_de_heatmap_fixed.pdf")))
...
main <- function(gse_id = NULL) {
  if (is.null(gse_id)) {
    ...
    if (length(args) > 0) {
      ...
      stop("Please provide a valid GSE ID. For example: Rscript differential_expression_fixed.R GSE223515")
    }
  }
